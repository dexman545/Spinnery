...
import static spinnery.registry.NetworkHandler.SLOT_CLICK_PACKET;
import static spinnery.registry.NetworkHandler.SLOT_DRAG_PACKET;
import static spinnery.registry.NetworkHandler.createSlotClickPacket;
import static spinnery.registry.NetworkHandler.createSlotDragPacket;
...
public class WSlot extends WAbstractWidget {
	...
	public void draw() {
		...
		BaseRenderer.getItemRenderer().renderGuiItemOverlay(Minecraft.getInstance().textRenderer, stackA, 1 + x + (sX - 18) / 2, 1 + y + (sY - 18) / 2, stackA.getCount() == 1 ? "" : withSuffix(stackA.getCount()));
